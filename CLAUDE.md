...
# 🗒 변경 이력 (최신이 위 / 한 줄씩)
- 2026-07-10 | 김운정 인스타그램 계정 변경 @woonjvng → @oonjvng — 푸터(app.js)·members.html href/표기, PROJECT_SPEC·제출_기획서 문서 표기 일괄 갱신, 캐시버스터 v=20260710e | 헤드리스로 푸터 렌더 확인
- 2026-07-10 | 2차 모바일 피드백 6종(IMG_1921~1926) — ①stores 본문 행간 확대(주소 1.75·연락처 1.7, 블록 간격 34px, 답답함 해소) ②모바일 메뉴 화살표를 우측 끝에서 40px 안쪽으로 ③홈 Feature 리스트 상하 여백 대칭(h4 위 마진 제거, 패딩 22px) ④푸터 링크드인·노션 URL 한 줄화(모바일 11.5px+nowrap, 노션 표시 텍스트에서 https://·쿼리 제거—href는 그대로) ⑤discover 활성 탭 밑줄을 구분선에 밀착(8px 뜨던 간격 제거, 웹·모바일 공통) ⑥festival 상단 문구 "…놀고싶다/면," 꺾임 수정(max-width 해제+14px+keep-all) | 각 항목 390px 스크린샷 검증, 21페이지 스모크 통과, 캐시버스터 v=20260710d
- 2026-07-10 | 모바일 전면 개선 6종(아이폰 스크린샷 피드백) — ①홈 Feature: 모바일 상하 패딩 통일·이미지-리스트 사이 과대 여백(≈90px) 축소·이미지 340px ②모바일 메뉴 드롭다운 화살표를 크고 진하게 우측 정렬 ③products: 모바일 히어로를 타탄 밴드 없는 중앙 사진 크롭(hero-mobile.jpg, picture 분기)으로 풀블리드+활성 탭 언더라인을 라벨 중앙 64px로 ④discover: 탭 3개 한 줄 복구+콜라주를 높이 620px 가로 스와이프(축소로 작아지던 문제)+안내 문구 ⑤festival: 산수유 카드를 도자기·쌀과 동일 구조(설명→선→제목+화살표+링크)로 통일(웹·모바일 모두, 링크가 제목에 붙던 문제)+desc break-all→keep-all(문단 밀림)+h2 keep-all ⑥stores: 히어로 250px·오버레이 0.8로 사진이 잘려 보이던 것을 460px(모바일 400px)+가벼운 그라디언트+이미지 절대배치(height:100%가 340px로 풀리던 % 기준 문제 회피)로 사진 중심 재구성+모바일 행간 1.5 통일 | 21페이지 스모크(콘솔 에러 0)·5개 폭 오버플로 0, 캐시버스터 v=20260710c
...
- 2026-07-10 | 모바일 우측 흰 여백 안전망 보강 — `overflow-x: hidden` 폴백(iOS 16 미만 Safari는 clip 미지원) + `overscroll-behavior-x: none`(바운스로 옆 배경 노출 방지), 캐시버스터 `?v=20260710` 일괄 갱신 | 360/375/390/414/430px 5개 폭 요소 단위 측정에서 오버플로 0 (실기기 잔존 시 = 폰 캐시, 새로고침 필요)
- 2026-07-10 | Gap 분석(PDCA Check) 실행 — 설계 문서 대비 일치율 약 94%, 상세는 `docs/03-analysis/js.analysis.md` | High 갭 없음, Med 1건(login/signup 푸터 없음 = 의도 여부 결정 필요)
- 2026-07-09 | 모바일 메뉴 후속 수정(아이폰 실기기 스크린샷 기반) — ①홈 최상단(투명 헤더)에서 메뉴 열면 "투명 헤더=흰 글자" 규칙이 흰 패널 위에 그대로 적용돼 항목이 안 보이던 것: `.navbar.navbar-mobile` 오버라이드로 열림 상태에선 항상 진한 색+로그인 버튼 검정 복원 ②back-to-top(z-index 99999 원본)이 메뉴 패널 위에 뜨던 것: app.js가 body에도 nav-open 토글 → 열림 동안 숨김 ③전 페이지 custom.css/app.js 링크에 `?v=20260709` 캐시버스터(폰 캐시로 구버전 보이던 문제) | IMG_1911/1912 재현 조건(투명 헤더에서 열기) 테스트로 링크색·버튼·back-to-top 숨김 확인
- 2026-07-09 | 모바일 메뉴 오버레이가 스크롤 후 상단 남색 띠로만 보이던 버그 수정 — `header-scrolled`의 backdrop-filter가 fixed 오버레이(.navbar-mobile)의 containing block을 헤더(64px)로 바꿔 갇히던 것. 메뉴 열림 동안 `#header.nav-open`으로 블러 해제(app.js 토글+custom.css), 오버레이 색도 템플릿 남색→브랜드 블랙(rgba(16,16,16,.6)) | 스크롤 상태 강제 재현 테스트로 열림 시 backdrop none·오버레이 844px 전체 덮음·닫힘 시 블러 복구 확인
- 2026-07-09 | 모바일 최적화 6종 — ①메뉴 버튼: main.js가 리스너 붙일 때 헤더가 아직 미주입이라 죽어 있던 것을 app.js `initMobileNav`(document 위임)로 해결+열림 상태 X 흰색·모바일 패널 밑줄 제거 ②홈 CTA: 모바일(≤767px)에서 배너를 우측 크롭(400px cover)+HTML 오버레이 문구·버튼으로 교체(박힌 글자 축소 문제) ③products 탭: 모바일에서 flex:1 3등분(좌측 쏠림·우측 빈칸 해소) ④products 히어로: 템플릿 section 기본 패딩(60px) 제거로 헤더 밀착 ⑤가격+Get now를 `.pv-card__body` absolute로 카드 내부 하단 고정(웹·모바일 동일, media padding-bottom으로 이미지와 분리) ⑥가로 스크롤 흰 여백: 원인인 홈 features `.row.g-5` 거터를 모바일에서 1.5rem으로 축소+html/body `overflow-x: clip` 안전망, product-detail 액션 버튼 좁은 화면 패딩 축소 | 21개 페이지 390px 오버플로 0 확인, 데스크톱 1460px 회귀 확인
- 2026-07-02 | 커머스 흐름 실데이터화 — app.js에 상품 카탈로그(VW.PRODUCTS, 가격 단일 출처)+장바구니(vwCart)·위시(vwWish)·주문(vwOrders) localStorage 모듈 추가(window.VW 공개), products 카드→`product-detail.html?id=` 연결(쌀 4kg 24,000/10kg 55,000 분리), 상세=?id 분기(쌀 중량 옵션 구현)+ADD TO CART/Get now 실저장, cart/checkout=vwCart 렌더(수량·삭제 영속, 빈 장바구니 처리), checkout 받는 분=로그인 사용자 자동입력+결제 시 주문 생성, order-complete=실주문 번호·상품·금액·배송지, mypage=로그인 사용자 이름·이메일 표시(비로그인 시 login으로)+위시·장바구니·주문내역 탭 실데이터+로그아웃 실동작 | "비비안" 하드코딩·상품/가격 불일치(주문완료 120,000 vs 주문서 324,000 등) 문제 해소
- 2026-07-02 | 축제(icheon-festival) 히어로 최적화 — 방금 추가됐던 중복 오버라이드(`.copy/.title` width·text-align, `.ic-hero-nav` 재선언 padding 52/44px) 제거하고 공통 `.ic-hero-nav` 규칙에만 의존, 히어로 아래 여백을 padding-bottom 80+nav margin 34(≈114px)에서 48px로 축소해 맛집·놀거리와 동일한 간격으로 | 히어로가 붕 떠 보이던 문제 해소(중앙정렬은 공통 규칙으로 데스크톱·모바일 모두 정확히 맞음을 측정 확인)
- 2026-06-18 | products ACC 왼쪽 카드 `acc-1.jpg` 검은 배경을 acc-2와 동일한 밝은 회색(#EDEDED)으로 통일(PIL 플러드필: 외곽+링 안쪽 큰 영역만 채우고 각인 텍스트는 보존) | 두 목걸이 카드 배경이 검정/흰색으로 따로 놀던 문제 해소
- 2026-06-18 | products rice 10kg 이미지를 잘리지 않은 전체 봉투(파란 10kg, 800×1000) 원본으로 교체 | 오른쪽 쌀 카드 이미지가 우/하단 잘려 보이던 문제 해소(4kg과 동일 프레이밍)
